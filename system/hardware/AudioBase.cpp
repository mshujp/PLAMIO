...
bool AudioBase::playSound(const Sound* sound, float gain)
{
    ...
    for (uint16_t i = 0; i < sound->stepCount; ++i)
    {
        ...
        const bool completed = toneSamples(
            ...
            totalSamples,
            writtenSamples,
            std::clamp(step.startVolume, 0.0f, 1.0f) * clampedGain,
            std::clamp(step.endVolume, 0.0f, 1.0f) * clampedGain
        );
...
bool AudioBase::playMusicStep()
{
...

    {
        ...
        localMusicVolume = localMusic == nullptr ? 0.0f : std::clamp(localMusic->gain, 0.0f, 1.0f);
    }
