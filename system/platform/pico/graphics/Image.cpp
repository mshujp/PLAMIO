...
ImageLayout calculateImageLayout(uint32_t sourceWidth, uint32_t sourceHeight, uint16_t outputWidth, uint16_t outputHeight, Image::ImageFit fit)
{
    ImageLayout layout;
...
public:
    ...
    bool decodeJpeg(const uint8_t* data, uint32_t size, Image::ImageFit fit)
    {
        ...
        const bool decoded = sprite.drawJpg(
            data,
            size,
...
Image::ImageData* PRUZEA::Image::loadJpeg(const uint8_t* jpegData, uint32_t jpegSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit)
{
    return load(jpegData, jpegSize, outputWidth, outputHeight, fit, false);
}

Image::ImageData* PRUZEA::Image::loadPng(const uint8_t* pngData, uint32_t pngSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit)
{
