...
public:
    ...
    virtual Image* loadJpeg(const uint8_t* jpegData, uint32_t jpegSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit) { return nullptr; }
    virtual Image* loadPng(const uint8_t* pngData, uint32_t pngSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit) { return nullptr; }
    ...
    // To specify a different transparent color, scale, or flip direction, use drawSprite() with Image::getBuffer(), getWidth(), and getHeight().
